# 介绍
一个简单易用的badgeview，支持自定义背景色，字体，文字颜色。通过扩展UITabbar，任意显示在某个tabbarItem。如下图
...
功能点
支持手写frame和autolayout，支持在IB中直接设置bgColor，textColor，badgeValue
自动计算size，手写frame时，只需调用sizetoFit，autolayout设置instrinsicContentSize为placeholder。具体可以看demo。
实现
BadgeView
BadgeView的实现比较简单，View里面加个label。
